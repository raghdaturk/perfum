$(document).ready(function() {
    ...
    $(".click").click(function() {
        $(this).toggleClass('active').parent().next('.expand').slideToggle();
    ...
    (function($) {
        $('.spinner .btn:first-of-type').on('click', function() {
            $('.spinner .spinner-text').val(parseInt($('.spinner .spinner-text').val(), 10) + 1);
        });
        $('.spinner .btn:last-of-type').on('click', function() {
            if ($('.spinner .spinner-text').val() > 1) { $('.spinner .spinner-text').val(parseInt($('.spinner .spinner-text').val(), 10) - 1); }
        });
    })(jQuery);
